  ...
  async ({ userId, message }) => {
    ...
    try {
      ...
      const { text } = await ai.generate({
        prompt: `You are the Kitchen Orchestrator at Indian Grill restaurant. Your job is to understand customer requests and provide helpful responses.
...
After your initial response, I will route the request to the appropriate agent based on your analysis.`,
      ...
      if (lowerMessage.includes('menu') || lowerMessage.includes('what') || lowerMessage.includes('serve')) {
      ...
      } else if (lowerMessage.includes('order') || lowerMessage.includes('want') || 
                 lowerMessage.includes('take') || lowerMessage.includes('i\'ll have') ||
        ...
        if (result.success) {
          ...
          if (chefResult.success) {
            return {
              success: true,
              intent: 'PlaceOrder',
              message: `${text}\n\nOrder placed successfully! Your ${result.dish} is now being cooked and will be ready in approximately 15-20 minutes. Order ID: ${result.orderId}`,
              userId,
              orderId: result.orderId,
              timestamp: new Date().toISOString()
            };
        ...
        return {
          success: true,
          intent: 'Fallback',
          message: `${text}\n\nI'm here to help! You can ask me about our menu, place an order, or check your order status.`,
          userId,
          timestamp: new Date().toISOString()
        };
      }
